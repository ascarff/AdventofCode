with open('4-input.txt','r') as f:
    ...
    def find_winning_board():
        ...
        return(winning_boards, winning_nums)

    winning_boards=[]
    winning_nums=[]
    winning_boards, winning_nums=find_winning_board()
    
    last_board = winning_boards[-1]
    last_num = winning_nums[-1]
    print("Last board =", last_board, ". Last num =", last_num)
    
    total_remaining = 0
    for row in range(5):
        for col in range(5):
            if boards[last_board][row][col]!=-999:
                total_remaining+=int(boards[last_board][row][col])
    
    final_score=int(total_remaining)*int(last_num)
    print("Total remaining =", total_remaining, ". Winning number =", last_num, ". Final score =", final_score)
